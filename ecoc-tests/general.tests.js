...
const ecoAssert = require('../test/utils/assert');
const { getEcocRPCAddress, getDefaultEcocAddress, getWalletPassphrase, isWalletEncrypted } = require('../test/utils');

const { assert } = chai;

console.log(`Your ecochain RPC address is ${getEcocRPCAddress()}`);
console.log(`Default ecochain address is ${getDefaultEcocAddress()}`);

describe('Ecoweb3', () => {
  const ECOC_ADDRESS = getDefaultEcocAddress();
  let ecoweb3;
    ...
    it('returns an array of address groupings', async () => {
      ...
      if (!_.isEmpty(res)) {
        ...
        if (!_.isEmpty(innerArr)) {
          ...
          ecoAssert.isEcocAddress(item[0]);
