...
class Encoder {
  ...
  /**
   * Converts an ecochain or hex address to a padded hex string.
   * @param {string} address The ECOC/hex address to convert.
  ...
  static validateMethodAndArgs(abi, methodName, args = []) {
	...
	throw Error('Number of arguments supplied does not match ABI method args. Should be '+methodObj.inputs.length+
      ' but '+args.length+' arguments where provided');
    }
